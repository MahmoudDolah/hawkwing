...
#[cfg(feature = "audio")]
fn cmd_play(db: Arc<Database>, artist: String, title: String) -> Result<()> {
    ...
    let Source::LocalFile(ref filepath) = best.source;
...
fn cmd_play(_db: Arc<Database>, _artist: String, _title: String) -> Result<()> {
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Play { artist, title } => {
            cmd_play(Arc::clone(&db), artist, title)?;
        }
    }
